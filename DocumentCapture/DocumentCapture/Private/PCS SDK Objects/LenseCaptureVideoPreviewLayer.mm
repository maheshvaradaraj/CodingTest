//
//  LenseCaptureVideoPreviewLayer.m
//  Paycasso
//
...
@interface LenseCaptureVideoPreviewLayer ()
...
@property (weak, nonatomic) UIBlurEffect *blur;
...
@implementation LenseCaptureVideoPreviewLayer
...
//        _targetLayer.lineWidth = self.frame.size.width * 0.006831; // 2 //self.frame.size.width * 0.002415; // 1 //self.frame.size.width * 0.016908 ; // 7
        _targetLayer.lineCap = kCALineCapRound;
        _targetLayer.strokeColor = [UIColor orangeColor].CGColor;
        _targetLayer.hidden = YES;
        ...
        _animator = [[UIViewPropertyAnimator alloc] initWithDuration:2.0 curve:UIViewAnimationCurveEaseOut animations:^{
        ...
        _blurMaskLayer.lineWidth = self.frame.size.width * 0.004831; // 2 //self.frame.size.width * 0.009662; //4
        ...
        [self addSublayer:_blurView.layer];
...
- (void)prepareTargetWithPoints:(CGPoint)tl tr:(CGPoint)tr br:(CGPoint)br bl:(CGPoint)bl {
...
//    NSLog(@"fdfd");
    if (_resetAnimation) {
        ...
        self.animator.fractionComplete = _blurIntensity;
    }

    if (_isShowDocument && !_src.empty()) {
        // Background animation
        _blurIntensity += (_topSizeFactor / self.frame.size.width) * 3.0 * displayLink.duration;
        float maxBlur = 0.75;
        _blurIntensity = MIN(_blurIntensity, maxBlur);
        
        if (_blurIntensity >= maxBlur) {
            _blurIntensity = maxBlur;
        ...
        _targetLayer.opacity -= 10.0 * displayLink.duration;
        ...
        if (_canMoveTarget) {
            ...
            [self prepareTargetWithPoints:_quadrilateral.tl tr:_quadrilateral.tr br:_quadrilateral.br bl:_quadrilateral.bl];
    ...
    if (_isShowQuadrangle) {
        ...
        _blurIntensity += 1.00 * displayLink.duration; // 3.00
...
//        _targetLayer.opacity = 0.8;
        ...
        _blurIntensity -= 2.0 * displayLink.duration; // 3.00
...
- (void)reset {
    ...
    if (!_canResetAnimation) {
        _blurIntensity = 0; 
        _animator.fractionComplete = _blurIntensity;
...
- (void)showWithQuadranglePoints:(std::vector<cv::Point>)points color:(UIColor *)color expectedDocument:(dc::ExpectedDocument)expectedDocument {
    ...
    [self prepareTargetWithPoints:tl tr:tr br:br bl:bl];
}
